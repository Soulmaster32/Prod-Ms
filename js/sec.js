/**
 * ============================================================================
 * QUALITY GROUP | MS SECTION - CLIENT-SIDE SECURITY ENGINE (sec.js) v3.0
 * ============================================================================
 * Features:
 * - Anti-Screenshot Defense: Traps PrintScreen, Win+Shift+S, and Cmd+Shift+3/4/5
 * - Instant Screen Blanking: Obscures DOM for 800ms during screenshot attempts
 * - Clipboard Poisoning: Overwrites clipboard with a legal warning text on capture
 * - Privacy Shield Overlay: Blurs dashboard when window loses focus (Anti-Snipping)
 * - Blocks Right-Click Context Menu across the entire portal
...
(function () {
    ...
    const CONFIG = {
        enabled: true,
        adminPasscode: 'QA-ADMIN-2026',  // Passcode to unlock: QASecurityEngine.unlock('QA-ADMIN-2026')
        ...
        enablePrivacyShield: true,        // Blur screen when window loses focus (blocks background snipping tools)
    ...
    function initSecurity() {
        ...
        printConsoleWarning();
        console.log('%c🛡️ QA MS Section Security & Anti-Screenshot Engine Armed', 'background: #e11d48; color: #fff; font-weight: bold; padding: 4px 8px; border-radius: 4px;');
    }

    /**
     * Inject CSS rules for selection locking and print protection
     */
    function injectSecurityStyles() {
        ...
        styleEl.innerHTML = `
            /* Disable Selection Across Webpage */
    ...
    /**
     * Inject Privacy Shield (Anti-Snipping & Window Blur Overlay)
     */
        ...
        shield.innerHTML = `
            <div class="max-w-md mx-auto">
                ...
                <p class="text-xs sm:text-sm text-slate-400 leading-relaxed mb-8">
                    To prevent background screen capture and protect proprietary MS Section data, dashboard contents are automatically obscured when the portal window loses focus.
                </p>
                <div class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-slate-200 text-xs font-bold shadow">
                    ...
                    <span>Click anywhere in this window to resume viewing</span>
                </div>
            </div>
        `;
    ...
    /**
     * Attach Anti-Screenshot Engine & Privacy Shield Listeners
     */
    function attachAntiScreenshotEngine() {
        // 1. Window Focus & Blur Defense (Protects against Windows Snipping Tool / Mac Snip)
        if (CONFIG.enablePrivacyShield) {
            window.addEventListener('blur', function () {
                if (isUnlocked) return;
                // Engage shield when user switches tabs or launches OS snipping tool
        ...
        // 2. Trapping Screenshot Key Combinations
        ...
        window.addEventListener('keydown', function (e) {
            ...
            if (keyCode === 44 || key === 'printscreen' || (e.metaKey && e.shiftKey && ['3','4','5','s'].includes(key))) {
                // Instantly flash privacy shield to corrupt the captured image!
                flashScreenBlank();
            }
        }, true);
    }
    ...
    function executeScreenshotCountermeasures(reason) {
        ...
        // Poison Clipboard: Overwrite captured image with a warning string
    ...
    function logSecurityIncident(actionName) {
        try {
            ...
            hist.unshift({
                ...
                details: `Unauthorized screenshot or data extraction attempt was blocked by sec.js engine.`
            });
    ...
    window.QASecurityEngine = {
        ...
        getStatus: function () {
            return {
                armed: !isUnlocked,
                version: '3.0.0-PROD',
                activeProtocols: ['ScreenshotTrap', 'ScreenBlankFlash', 'ClipboardPoison', 'WindowBlurShield', 'RightClickBlock', 'DevToolsTrap', 'SelectionLock', 'PrintProtect']
            };
        }
    };
